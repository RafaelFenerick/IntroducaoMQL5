...
input double             LimiteInf      = 30;          // Limite Inferior
...
// Criação dos manipulador
   ...
   handle2 = iRSI(Ativo2, _Period, Periodo, Preco);
   handle3 = iRSI(Ativo3, _Period, Periodo, Preco);
...
int Sinal()
  {
   double ifr1[], ifr2[], ifr3[];
   
   ArraySetAsSeries(ifr1, true);
   ArraySetAsSeries(ifr2, true);
   ArraySetAsSeries(ifr3, true);
   
   CopyBuffer(handle1, 0, 1, 2, ifr1);
   CopyBuffer(handle2, 0, 1, 2, ifr2);
   CopyBuffer(handle3, 0, 1, 2, ifr3);
   
   bool compra = true;
   bool venda = true;
   
   compra &= ifr1[0] < LimiteInf;
   compra &= ifr2[0] < LimiteInf;
   compra &= ifr3[0] < LimiteInf;
   
   venda &= ifr1[0] > LimiteSup;
   venda &= ifr2[0] > LimiteSup;
   venda &= ifr3[0] > LimiteSup;
